  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-slate-400 p-6">
        ...
        <p className="text-sm font-medium">Loading job details for editing...</p>
      </div>
    );
  }
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-blue-950/60 via-indigo-950/40 to-slate-900 p-8 border border-blue-500/20 shadow-2xl">
        <div className="relative z-10 max-w-3xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/20 text-blue-300 text-xs font-semibold mb-3">
            ...
            <span>Edit Job Specification</span>
          </div>
          <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
            Update Job Opening #{jobId}
          </h1>
          <p className="text-sm text-slate-300 mt-2">
            Modify title, description, skills, salary range, or change status (active/closed/draft).
          </p>
        </div>
      </div>

      {/* Main Job Form */}
      <form onSubmit={handleSubmit} className="bg-slate-900/80 backdrop-blur-xl p-8 rounded-3xl border border-slate-800 space-y-8 shadow-2xl max-w-4xl mx-auto">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-300">
                Job Status <span className="text-rose-400">*</span>
              </label>
              <select
                name="status"
        ...
        <div className="flex items-center justify-end gap-4 pt-6 border-t border-slate-800">
          <Link
            to="/recruiter/jobs"
            className="px-6 py-3 bg-slate-800 hover:bg-slate-700 text-xs font-bold text-slate-300 rounded-xl transition-all"
          >
            Cancel
          </Link>
          ...
          <button
            type="submit"
            disabled={saving}
            className="px-8 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-xs font-extrabold text-white rounded-xl shadow-lg shadow-blue-500/25 transition-all flex items-center gap-2"
          >
